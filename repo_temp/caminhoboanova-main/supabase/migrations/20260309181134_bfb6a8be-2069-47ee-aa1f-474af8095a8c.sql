-- Update the ranking function to apply weekend reduced points for devotionals
...
BEGIN
  ...
  devotional_pts AS (
    SELECT dp.user_id, 
      SUM(CASE 
        WHEN EXTRACT(DOW FROM dp.completed_at) IN (0, 6) THEN 2  -- Weekend = 2 pts
        ELSE 5  -- Weekday = 5 pts
      END) AS pts
