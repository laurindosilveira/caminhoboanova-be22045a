...
BEGIN
  ...
  WITH lesson_points AS (
    SELECT lr.user_id, COUNT(DISTINCT lr.lesson_id) AS cnt, COUNT(DISTINCT lr.lesson_id) * 20 AS pts
  ...
  devotional_pts AS (
    SELECT dp.user_id, COUNT(*) AS cnt,
      SUM(CASE 
        WHEN EXTRACT(DOW FROM dp.completed_at) IN (0, 6) THEN 2
        ELSE 5
      END) AS pts
  ...
  attendance_pts AS (
    SELECT a.user_id, COUNT(*) AS cnt, COUNT(*) * 10 AS pts
    FROM attendance a
  ...
  worship_pts AS (
    SELECT wa.user_id, COUNT(*) AS cnt, COUNT(*) * 5 AS pts
  ...
  activity_pts AS (
    SELECT up.user_id, COALESCE(SUM(act.points), 0) AS pts
  ...
  challenge_pts AS (
    SELECT cp.user_id, COUNT(*) * 15 AS pts
  ...
  SELECT
    p.user_id,
    p.full_name,
    (COALESCE(lp.cnt, 0) + COALESCE(dp.cnt, 0) + COALESCE(atp.cnt, 0) + COALESCE(wp.cnt, 0))::bigint AS completed_count,
    (COALESCE(lp.pts, 0) + COALESCE(dp.pts, 0) + COALESCE(atp.pts, 0) + COALESCE(ap.pts, 0) + COALESCE(cb.pts, 0) + COALESCE(wp.pts, 0) + COALESCE(achp.pts, 0) + COALESCE(chp.pts, 0))::bigint AS faith_points
  FROM profiles p
