...
import { ChevronDown, ChevronUp, Search, X, Users2, BookOpenCheck, ChevronLeft } from "lucide-react";
import SimpleStat from "@/components/ui/SimpleStat";
...
type Participant = {
  ...
  turma_id?: string | null; confirmation_year?: number | null; church_id?: string | null;
  completed_lesson_count?: number; completed_devotional_count?: number; completed_event_count?: number;
};
...
const AREA_ICONS: Record<string, string> = {
  "Área 1": "🌳", "Área 2": "🔥", "Área 3": "💧", "Área 4": "🌬️",
  "Jovens": "🚀", "Adolescentes": "🎯", "Kids": "🎈",
};
...
export default function AdminDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [{ data: churchesData }, { data: profilesData }, userResult] = await Promise.all([
      supabase.from("churches").select("id, name, slug, city, state, is_active").order("name"),
    ...
    const profilesList = (profilesData ?? []).filter(p => {
    ...
    activitiesQuery = scopedChurchId
      ? activitiesQuery.or(`church_id.is.null,church_id.eq.${scopedChurchId}`)
      : activitiesQuery.is("church_id", null);
    ...
    const churchFilter = `church_id.is.null,church_id.eq.${scopedChurchId}`;
    progressQuery = scopedChurchId ? progressQuery.or(churchFilter) : progressQuery.is("church_id", null);
    lessonQuery = scopedChurchId ? lessonQuery.or(churchFilter) : lessonQuery.is("church_id", null);
    devotionalQuery = scopedChurchId ? devotionalQuery.or(churchFilter) : devotionalQuery.is("church_id", null);
    attendanceQuery = scopedChurchId ? attendanceQuery.or(churchFilter) : attendanceQuery.is("church_id", null);
    ...
    setPlans(plansMap);
    setChurchLimit(subResult.data?.member_limit ?? null);
    setLoading(false);
  ...
  const filteredParticipants = useMemo(() =>
    selectedTurma
      ? isAreaScope
        ? participants.filter(p => p.area === selectedTurma.area)
        : participants.filter(p => p.turma_id === selectedTurma.id)
      : participants,
  ...
  if (!selectedTurma) {
    return (
      <TurmaSelector
        ...
        selectedChurchName={selectedChurch?.name ?? profile?.churches?.name ?? ""}
  ...
  return (
    <div className="min-h-screen bg-background">
      <AdminHeader
        ...
        stats={{ ...stats, memberLimit: churchLimit, totalLabel: churchLimit ? `${stats.total}/${churchLimit}` : `${stats.total}` }}
        onSignOut={signOut}
      ...
      <div className="mx-auto flex w-full max-w-[1440px] items-start">
        ...
        <main className="min-w-0 flex-1 px-4 py-6 pb-24 sm:px-6 lg:px-8 lg:pb-10">
          {loading ? (
            ...
            {activeTab === "care" && (
              <LeaderCareDashboard
                area={selectedTurma.area}
                churchId={currentChurchId}
                turmaId={isAreaScope ? null : selectedTurma.id}
                participants={filteredParticipants.map(({ user_id, full_name, birth_date }) => ({ user_id, full_name, birth_date }))}
...
function TurmaSelector({
  ...
  const selectArea = (areaName: string) => {
    onSelectTurma({
      id: `area::${areaName}`, name: areaName, area: areaName,
      year: new Date().getFullYear(), is_active: true, description: null, church_id: selectedChurchId,
    });
  };

  const areasWithTurmas = useMemo(() => {
    const areaMap = new Map<string, { name: string; turmas: Turma[]; communities: string[]; icon: string }>();
    const areasInOrder = isSuper ? AREAS : [adminArea];

    areasInOrder.forEach(areaName => {
      areaMap.set(areaName, {
        name: areaName,
        turmas: [],
        communities: getCommunitiesForArea(areaName),
        icon: AREA_ICONS[areaName] ?? "📍",
      });
    });

    turmas.forEach(turma => {
      if (turma.area && areaMap.has(turma.area)) {
        areaMap.get(turma.area)!.turmas.push(turma);
      }
    });

    return Array.from(areaMap.values());
  }, [turmas, isSuper, adminArea]);

  const searchLower = turmaSearch.toLowerCase().trim();
  const filteredAreas = useMemo(() => areasWithTurmas.map(areaInfo => ({
    ...areaInfo,
    turmas: searchLower
      ...
      : areaInfo.turmas,
  })).filter(areaInfo => areaInfo.turmas.length > 0 || !searchLower), [areasWithTurmas, searchLower]);

  const totalParticipants = useMemo(() => new Set(participants.map(p => p.user_id)).size, [participants]);
      ...
      <header className="px-4 pt-8 pb-6" style={{ background: "var(--gradient-hero)" }}>
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className="w-12 h-12 rounded-xl bg-white/15 border-2 border-white/30 flex items-center justify-center">
              <span className="text-2xl">{isSuper ? "👑" : "✝️"}</span>
            </div>
            <div>
              <p className="text-primary-foreground/60 font-inter text-xs">
                {isSuper ? "Super Administrador" : "Líder de Discipulado"}
              </p>
              <h1 className="font-montserrat font-black text-primary-foreground text-xl uppercase tracking-tight">Seleção de Turma</h1>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            <SimpleStat icon={<Users2 className="w-5 h-5 text-cyan-300" />} label="Participantes" value={totalParticipants} />
            <SimpleStat icon={<BookOpenCheck className="w-5 h-5 text-cyan-300" />} label="Turmas Ativas" value={turmas.length} />
          </div>
        </div>
      </header>
        ...
        {loading ? <TabSkeleton /> : (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {isSuper && churches.length > 1 && (
                <div className="md:col-span-2 rounded-lg border border-border bg-card p-3 shadow-sm">
                  <label className="mb-1.5 block font-inter text-[11px] font-bold uppercase tracking-wide text-muted-foreground">
                    Filtrar por Igreja
                  </label>
                  <div className="relative">
                    <select
                      ...
                      className="h-11 w-full appearance-none rounded-lg border border-input bg-background px-3 pr-9 font-inter text-sm font-semibold text-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring"
                    >
                      {churches.map(church => (
              ...
              {turmas.length > 4 && (
                 <div className="relative md:col-span-2">
                  ...
                  <input
                    ...
                    className="w-full h-12 rounded-lg border border-input bg-background pl-10 pr-10 text-sm text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring font-inter"
                  />
                  {turmaSearch && (
                    <button
            ...
            {filteredAreas.map(areaInfo => {
              ...
              const areaParticipantsCount = participants.filter(p => p.area === areaInfo.name).length;

              return (
                <div key={areaInfo.name} className="space-y-3">
                  <button
                    onClick={() => {
                    ...
                    className={`w-full flex items-center gap-4 p-4 bg-card rounded-xl border-2 shadow-sm transition-all group ${
                      isExpanded ? "border-primary/50 bg-primary/5" : "border-border hover:border-primary/30"
                    }`}
                  >
                    <div className="w-12 h-12 rounded-lg bg-muted flex items-center justify-center text-2xl transition-transform group-hover:scale-110">
                      {areaInfo.icon}
                    </div>
                    ...
                    {areaInfo.turmas.length > 0 && !searchLower && (
                      isExpanded
                  ...
                  {isExpanded && (
                    <div className="pl-4 md:pl-6 space-y-2 animate-in slide-in-from-top-2 duration-300">
                      {areaInfo.turmas.length > 0 ? (
                        areaInfo.turmas.map(turma => (
                          <TurmaCard
                            key={turma.id}
                            turma={turma}
                            participantCount={participants.filter(p => p.turma_id === turma.id).length}
                            onSelect={() => onSelectTurma(turma)}
                          />
                        ))
                      ) : (
