...
BEGIN
  ...
  lesson_stats AS (
    SELECT lr.user_id, COUNT(DISTINCT lr.lesson_id) as cnt, COUNT(DISTINCT lr.lesson_id) * _lesson_pts as pts
  ...
  att_stats AS (
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE c.value = e.type 
        ...
        AND (c.area IS NULL OR c.area = ul.area::text)
      ORDER BY 
        CASE WHEN c.church_id = ul.church_id THEN 0 ELSE 1 END,
        CASE WHEN c.area = ul.area::text THEN 0 ELSE 1 END
      LIMIT 1
    ) cet ON true
  ...
  wor_stats AS (
    SELECT wa.user_id, COUNT(*) as cnt, COUNT(*) * _wor_pts as pts
  ...
  SELECT 
    ul.user_id,
    ul.full_name,
    (COALESCE(ls.cnt, 0) + COALESCE(ds.cnt, 0) + COALESCE(as2.cnt, 0) + COALESCE(ws.cnt, 0) + COALESCE(eas.cnt, 0))::BIGINT as completed_count,
    CAST(
      COALESCE(ls.pts, 0) + 
      COALESCE(ds.pts, 0) + 
      COALESCE(as2.pts, 0) + 
