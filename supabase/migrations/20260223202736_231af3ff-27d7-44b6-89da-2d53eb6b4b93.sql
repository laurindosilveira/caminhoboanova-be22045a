-- Update ranking function to include worship attendance at 5pts each
...
AS $function$
  ...
  SELECT
    ...
    (COALESCE(lp.pts, 0) + COALESCE(dp.pts, 0) + COALESCE(atp.pts, 0) + COALESCE(ap.pts, 0) + COALESCE(cb.pts, 0) + COALESCE(wp.pts, 0)) AS faith_points
  FROM profiles p
