...
BEGIN
  ...
  lesson_points AS (
    -- Count distinct lessons completed (removed church filter)
  ...
  devotional_pts AS (
    -- Sum devotional points (removed church filter)
    SELECT
      dp.user_id,
      SUM(
        COALESCE(
  ...
  attendance_pts AS (
    -- Sum attendance points (removed church filter)
    SELECT
      a.user_id,
      SUM(
        CASE
  ...
  achievement_pts AS (
    -- Achievement points (using achievement_unlocks and achievement_definitions)
    -- Joining by 'key' column
    SELECT au.user_id, SUM(COALESCE(au.bonus_points, ad.bonus_points, 0)) AS pts
    FROM public.achievement_unlocks au
    JOIN public.achievement_definitions ad ON ad.key = au.achievement_key
