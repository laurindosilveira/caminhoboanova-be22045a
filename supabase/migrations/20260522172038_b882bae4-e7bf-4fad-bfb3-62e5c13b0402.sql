...
BEGIN
  ...
  devotional_pts AS (
    -- Sum devotional points using awarded_points when available
    SELECT
      dp.user_id,
      SUM(
        COALESCE(
  ...
  activity_pts AS (
    -- Sum points for generic activities
  ...
  achievement_pts AS (
    -- Achievement points: handles both defined achievements and manual bonuses
    SELECT au.user_id, SUM(COALESCE(au.bonus_points, ad.bonus_points, 0)) AS pts
    FROM public.achievement_unlocks au
    LEFT JOIN public.achievement_definitions ad ON ad.key = au.achievement_key
